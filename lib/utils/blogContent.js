...
function isLoosePipeMarker(line) {
...
export function parseBlogBody(body = '') {
  ...
  while (index < lines.length) {
    ...
    const calloutMatch = trimmed.match(/^:::\s*callout\s+(.+)$/);
    if (calloutMatch) {
      flushParagraph(blocks, paragraph);
      blocks.push({ type: 'callout', text: calloutMatch[1].trim() });
      index += 1;
      continue;
    }

    if (isTableDivider(trimmed) || isLoosePipeMarker(trimmed)) {
      flushParagraph(blocks, paragraph);
      index += 1;
      continue;
    }
