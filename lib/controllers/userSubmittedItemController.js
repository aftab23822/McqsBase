...
import { notifyNewUserSubmission } from '../email/submissionNotify.js';
  ...
  try {
    ...
    if (type === 'interview') {
      ...
      if (!position || !sharedBy || !year || !department || !answer) {
        return { 
          success: false, 
          message: 'Position, shared by, year, department, and full interview details are required for interview submissions',
          status: 400
        };
      }
    ...
    void notifyNewUserSubmission(newSubmission.toObject());
...
export async function updateSubmissionContent(id, body) {
  try {
    const {
      question,
      options,
    ...
    if (correct_option !== undefined) updateData.correctAnswer = correct_option;
    if (correctAnswer !== undefined) updateData.correctAnswer = correctAnswer;
    if (answer !== undefined) updateData.answer = answer;
    if (detail_link !== undefined) updateData.detail_link = detail_link;
