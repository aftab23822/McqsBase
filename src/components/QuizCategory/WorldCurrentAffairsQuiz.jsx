  ...
  const [totalPages, setTotalPages] = useState(1);
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch(`/api/quiz/world-current-affairs?page=${currentPage}&limit=${quizPerPage}`);
  ...
  return (
    <div className="relative">
      <div className={isPageChanging ? 'opacity-60 pointer-events-none transition-opacity duration-300' : 'transition-opacity duration-300'}>
        <BaseQuiz quizData={displayData} title="World Current Affairs Quiz"
...
export default WorldCurrentAffairsQuiz; 