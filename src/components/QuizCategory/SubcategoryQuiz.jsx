...
function NavigationCard({ child, index, activeSlug }) {
  ...
  const href = `/${BASE_PATH}/${child.fullSlug}`;
  ...
  return (
    <Link href={href} className={`${baseClasses} ${stateClasses}`}>
      ...
      {isCurrent ? (
        <p className="mt-2 text-sm text-gray-600">You are viewing this topic.</p>
      ) : isActive ? (
        <p className="mt-2 text-sm text-gray-600">Currently exploring this branch.</p>
      ) : null}
    </Link>
  );
}
...
export default function SubcategoryQuiz({ subject, subcategory, subcategorySegments, initialPage }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch(`/api/quiz/${subject}/${subPath}?page=${currentPage}&limit=${quizPerPage}`);
  ...
  const title = `${humanize(subject)} — ${readablePath} Quiz`;
