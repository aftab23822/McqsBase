...
const OralPathologyMedicineMcqs = () => {
  ...
  const [totalPages, setTotalPages] = useState(1);
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch(`/api/mcqs/oral-pathology-medicine?page=${currentPage}&limit=${mcqsPerPage}`);
  ...
  return (
    <div className="relative">
      <div className={isPageChanging ? 'opacity-60 pointer-events-none transition-opacity duration-300' : 'transition-opacity duration-300'}>
        <BaseMcqs mcqsData={displayData}
          title="Oral Pathology and Medicine MCQs"
          currentPage={currentPage}
          setCurrentPage={setCurrentPage}
          totalPages={totalPages}
          mcqsPerPage={mcqsPerPage}
          subjectSlug="oral-pathology-medicine"
        />
      </div>
      {isPageChanging && (
...
export default OralPathologyMedicineMcqs; 