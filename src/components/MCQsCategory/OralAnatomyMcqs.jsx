  ...
  const [totalPages, setTotalPages] = useState(1);
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch(`/api/mcqs/oral-anatomy?page=${currentPage}&limit=${mcqsPerPage}`);
    ...
    <div className="relative">
      <div className={isPageChanging ? 'opacity-60 pointer-events-none transition-opacity duration-300' : 'transition-opacity duration-300'}>
        <BaseMcqs mcqsData={displayData}
          title="Oral Anatomy MCQs"
          currentPage={currentPage}
          setCurrentPage={setCurrentPage}
          totalPages={totalPages}
          mcqsPerPage={mcqsPerPage}
          subjectSlug="oral-anatomy"
        />
      </div>
      {isPageChanging && (
...
export default OralAnatomyMcqs; 