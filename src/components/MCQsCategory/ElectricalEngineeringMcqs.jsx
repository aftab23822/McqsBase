...
const ElectricalEngineeringMcqs = () => {
  ...
  const [totalPages, setTotalPages] = useState(1);
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch(`/api/mcqs/electrical-engineering?page=${currentPage}&limit=${mcqsPerPage}`);
    ...
    <div className="relative">
      <div className={isPageChanging ? 'opacity-60 pointer-events-none transition-opacity duration-300' : 'transition-opacity duration-300'}>
        <BaseMcqs mcqsData={displayData}
          title="Electrical Engineering MCQs"
...
export default ElectricalEngineeringMcqs; 