  ...
  const [totalPages, setTotalPages] = useState(1);
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch(`/api/mcqs/auditing?page=${currentPage}&limit=${mcqsPerPage}`);
  ...
  return (
    <div className="relative">
      <div className={isPageChanging ? 'opacity-60 pointer-events-none transition-opacity duration-300' : 'transition-opacity duration-300'}>
        <BaseMcqs mcqsData={displayData} title="Auditing MCQs"
